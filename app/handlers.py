...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if borrar_match:
        ...
        if fila:
            get_sheet().update(f"{col}{fila}", [[""]])
            if letra == 'O':
                get_sheet().update(f"F{fila}", [[""]])
    ...
    if match_o:
        ...
        get_sheet().update(f"E{fila}", [[monto]])
        get_sheet().update(f"F{fila}", [[comentario]])
    ...
    if match:
        ...
        get_sheet().update(f"{col}{fila}", [[monto]])
